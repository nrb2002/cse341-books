...
const getAllBookAuthors = async () => {
  const relationships = await bookAuthorsCollection.find({}).toArray();
...
const getAuthorsByBookId = async (bookId) => {
  const relationships = await bookAuthorsCollection.find({ bookId }).toArray();
...
const createBookAuthor = async (bookId, authorId) => {
  const existingRelationship = await getBookAuthor(bookId, authorId);
